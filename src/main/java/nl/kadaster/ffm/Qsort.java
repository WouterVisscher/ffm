...
public class Qsort {
  public static void main(String[] args) throws Throwable {
    ...
    FunctionDescriptor qsortDescriptor = FunctionDescriptor.ofVoid(ValueLayout.ADDRESS,
    ...
    FunctionDescriptor compareDescriptor = FunctionDescriptor.of(ValueLayout.JAVA_INT,
        ValueLayout.ADDRESS.withTargetLayout(ValueLayout.JAVA_INT),
        ValueLayout.ADDRESS.withTargetLayout(ValueLayout.JAVA_INT));

    // 7. Get a handle to the "compare" function
    MethodHandle compareHandle =
        MethodHandles.lookup().findStatic(Qsort.class, "compare", compareDescriptor.toMethodType());
    ...
    try (Arena offHeap = Arena.ofConfined()) {
      ...
      qsortHandle.invoke(arrayAddress, unsorted.length, ValueLayout.JAVA_INT.byteSize(),
          compareAddress);
